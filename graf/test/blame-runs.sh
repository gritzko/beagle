...
#  --- TLV: each hunk URI is the U-tagged commit:?<sha40> -------------
"$GRAF" --tlv 'blame:f.c' >"$TMP/tlv.bin" 2>/dev/null || fail "tlv blame failed"
grep -aq "commit:?$S1" "$TMP/tlv.bin" || fail "tlv: no commit:?$S1 URI"
grep -aq "commit:?$S2" "$TMP/tlv.bin" || fail "tlv: no commit:?$S2 URI"
