...
| `WEAVE.{h,c}` | Token-level file history as ONE interleaved-delta TLV stream (`u8b tlv`): `T <bytes>` token records interleaved with run-length `I <sorted u32 set>` (inserters) / `R <sorted u32 set>` (removers) provenance records — a token is alive iff its R-set is empty, spine iff `0 ∈` its I-set.  No lexer tag is stored.  `weavebld` (RLE-encoding builder: `WEAVEBldPut`) and `weavecur` (sequential cursor: `WEAVECurNext`) are the write/read primitives; `WEAVEAliveBytes` emits the alive stream; the heavy ops decode to transient BASS arrays (`WEAVE_DECODE` macro — acquire in the op's own `sane()` frame, NOT via `call()`, which would rewind BASS).  `WEAVEDiff` projects alive src hashlets vs nu, runs `BRAMu64s` + `NEILCleanup`/`Shift`/`Canon`, then composes (INS = nu tokens I={src}; DEL folds src into R; EQ/lift carry provenance).  `WEAVEMerge` diffs the FULL streams keyed by `RAPHash·K + min(I-set)`, `NEILCanon` only (no cleanup — it would split same-identity shared tokens), EQ-reconciles I=union/R=union, prefix-unions concurrent-identical inserts (full alive-containment guard), and a disjoint-tail rescue reconciles one-logical-token pairs (equal-set+opposite-aliveness, or proper-subset of I-sets) — ours-side precedes theirs-side so `WEAVEEmitMerged` frames conflicts ours-first.  Conflict marker bytes (`<<<<`/`||||`/`>>>>`) are render-time only (never stored).  `WEAVEEmitDiff`/`EmitFull` emit unified-diff/`cat:` hunks (tags flat `'S'` for now); `WEAVEEmitMerged` frames divergent regions.  KNOWN: a deep cross-side reconciliation ORDERING cascade (test/TRIANGLE.todo.md; 1/928 corpus). |
