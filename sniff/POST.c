...
static ok64 post_pd_cb(ulogreccp src, void *vctx) {
    ...
    //  Sub-mount bump (`put <sub>#<40-hex>`): fragment is a new gitlink
    //  sha, not a move destination.  These rows are AUTO-GENERATED by the
    //  post-order sub recursion (be's bepost_bump_sub), NOT user staging
    //  intent.  They must therefore NOT flip the level into selective
    //  mode — otherwise a dirty (or even clean) sub silently drops the
    //  level's own commit-all edits / poisons the commit (SUBS-001).
    //  Emit the put intent so post_classify_step still ADDs the bumped
    //  gitlink, but leave c->any_pd untouched (commit-all survives).
    //  Detected before the any_pd flip and before the move-form check;
    //  the 40-hex-valid fragment is what distinguishes a bump from a
    ...
    //  Any other put/delete row is genuine user staging intent: it flips
    //  this level's POST from commit-all (implicit) into selective mode.
    c->any_pd = YES;
    ...
    //  (Sub-mount bump `put <sub>#<40-hex>` already handled above the
    //  any_pd flip — it returned early without flipping selective mode.)
