...
# Snapshot .be/wtlog length so we can assert no new row is appended.
...
# `.be/wtlog` did NOT grow — overwrite is no-staging (no `get` row).
ROWS_AFTER=$(wc -l < .be/wtlog)
[ "$ROWS_BEFORE" = "$ROWS_AFTER" ] || {
    echo "single-file get appended a .be/wtlog row (was $ROWS_BEFORE, now $ROWS_AFTER)" >&2
    exit 1
}
