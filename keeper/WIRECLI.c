...
static ok64 wcli_be_to_wire(u8b out, u8csc be_branch) {
    ...
    //  URI-002: the query-bang (`?X!` whole-branch modifier) is never
    //  part of a git refname — shed it via the uniform debanger before
    //  composing the wire ref.
    (void)DOGDebangSlice(name);
...
ok64 WIREFetch(u8csc remote_uri, u8csc want_ref) {
    ...
    //  URI-002: shed a trailing `!` via the uniform debanger — the
    //  PATCH whole-branch scope modifier (DOG_BANG_QUERY) rides on the
    //  query (`?master!`) but is never part of the wire refname the peer
    //  advertises.
    (void)DOGDebangSlice(effective_ref);
