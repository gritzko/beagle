...
static u64 keep_global_max_seqno(void) {
    u64 max = 0;
    a_path(bedir);
...
static u32 keep_global_max_pack_file_id(void) {
    u32 max = 0;
    a_path(bedir);
    (void)HOMEBranchDir(bedir, NULL);
...
//  An allocated `packs` registry indicates KEEPOpen has populated the
//  singleton and KEEPClose hasn't yet released it (zerop'd on close).
static b8 keep_is_open(void) { return !BNULL(KEEP.packs); }
...
ok64 KEEPOpenBranch(u8cs branch, b8 rw) {
    ...
    //  HOMEOpenBranch above).  Readers consult `u8bDataC(HOME.cur_branch)`.
    ...
    a_path(trunkdir);
    call(HOMEBranchDir, trunkdir, NULL);
    ...
    if (rw) {
        a_path(leafdir);
        call(HOMEBranchDir, leafdir, HOME.cur_branch);
...
ok64 KEEPOpen(b8 rw) {
    ...
    return KEEPOpenBranch(trunk, rw);
}
...
ok64 KEEPResolveTree(uricp target, sha1 *tree_sha) {
    ...
    if (u8csEmpty(target->fragment) && u8csEmpty(target->query) &&
        !BNULL(HOME.cur_sha) && DOGIsFullSha(u8bDataC(HOME.cur_sha))) {
        uri synth = *target;
        a_dup(u8c, cur_hex, u8bDataC(HOME.cur_sha));
...
ok64 KEEPImport(u8cs pack_path) {
    ...
    u32 file_id = keep_global_max_pack_file_id() + 1;
    a_path(kdir);
    call(HOMEBranchDir, kdir, HOME.cur_branch);
    {
        a_pad(u8, dst, 1024);
...
//  Drain one pkt-line from `buf`, refilling from `in_fd` via FILEDrain
//  on NODATA.  Mirrors recv_read_pkt / wcli_read_pkt but stays local
//  to KEEPPush so the file's other callers don't pick it up.
static ok64 keep_push_drain_pkt(int in_fd, u8b buf, u8cs adv, u8csp line) {
    for (;;) {
        ok64 o = PKTu8sDrain(adv, line);
        if (o != NODATA) return o;
        if (!u8bHasRoom(buf)) return KEEPFAIL;
        u8s fill;
        u8sFork(u8bIdle(buf), fill);
        ok64 fr = FILEDrain(in_fd, fill);
        if (fr == FILEEND) return KEEPFAIL;
        if (fr != OK) return KEEPFAIL;
...
ok64 KEEPPush(u8csc host, u8csc path, char const *ref,
    ...
    // it; the caller provided old_hex authoritatively.
    {
        ...
        u8cs line = {};
        for (;;) {
            ok64 o = keep_push_drain_pkt(rfd, rbuf_b, adv, line);
            if (o == PKTFLUSH) break;
