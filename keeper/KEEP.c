...
// --- N-way path-list merge cursor ---
//
//  See KEEP.h for the contract.  Single-pass min-finder + bitmask
//  collector: as we walk the inputs we update `min_line` whenever
//  we see a strictly smaller head (resetting `mask` to just this
//  input's bit) or extend `mask` whenever we see an equal head.
//  After the pass, `mask` flags every contributor; a separate loop
//  advances those contributors via u8csDrainLine.
ok64 KEEPu8ssDrain(u8css inputs, u8csp path_out, u64 *mask_out) {
    sane(path_out && mask_out);
    *mask_out = 0;
    path_out[0] = NULL;
    path_out[1] = NULL;
    ...
    for (size_t i = 0; i < n; i++) {
        u8cs *cur = $atp(inputs, i);
        if (u8csEmpty(*cur)) continue;

        //  Peek the i-th input's head line without consuming it.
        //  `peek` is a stack-local copy of the cursor's slice; the
        //  drain mutates `peek` only — the caller's cursor stays put.
        a_dup(u8c, peek, *cur);
        u8cs line = {};
        if (u8csDrainLine(peek, line) != OK) continue;

        if (u8csEmpty(min_line)) {
            min_line[0] = line[0];
            min_line[1] = line[1];
            mask = 1ULL << i;
        } else {
            int cmp = u8cscmp(&line, &min_line);
            if (cmp < 0) {
                min_line[0] = line[0];
                min_line[1] = line[1];
                mask = 1ULL << i;
    ...
    if (mask == 0) return KEEPNONE;

    //  Advance every contributor past the matched line + newline.
    //  Use the same drain helper so trailing-newline-less records
    //  are tolerated identically.
    for (size_t i = 0; i < n; i++) {
        if (!(mask & (1ULL << i))) continue;
        u8cs *cur = $atp(inputs, i);
        u8cs throwaway = {};
        (void)u8csDrainLine(*cur, throwaway);
    }

    path_out[0] = min_line[0];
    path_out[1] = min_line[1];
    *mask_out = mask;
    return OK;
}
